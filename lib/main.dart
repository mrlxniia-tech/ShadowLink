...
String getTitreNiveau(int lvl, String customTitle) { 
  if (customTitle.isNotEmpty) return customTitle;
  if(lvl >= 50) return "Légende"; if(lvl >= 30) return "Élite"; if(lvl >= 10) return "Vétéran"; if(lvl >= 5) return "Habitué"; return "Recrue"; 
}
...
class _RegisterPageState extends State<RegisterPage> {
  ...
  Future<void> creerCompte() async {
    ...
    try {
      ...
      await FirebaseFirestore.instance.collection('utilisateurs').doc(userCred.user!.uid).set({ 'email': emailController.text.trim(), 'pseudos': tousMesPseudos, 'amis': [], 'xp': 0, 'coins': 100, 'couleur': 'Cyan', 'enLigne': true, 'role': r, 'banni': false, 'warnings': 0, 'badge': '', 'customTitle': '', 'filtreInsultes': true, 'isLFG': false, 'isAFK': false, 'statut': 'Nouveau !', 'dateCreation': FieldValue.serverTimestamp() });
...
class _MainScreenState extends State<MainScreen> with WidgetsBindingObserver {
  ...
  String myRole = 'user'; String myAvatarUrl = ''; final String myUid = FirebaseAuth.instance.currentUser!.uid; String monPseudoGeneral = ''; String myBadge = '';
  ...
  Future<void> _chargerInfos() async { var doc = await FirebaseFirestore.instance.collection('utilisateurs').doc(myUid).get(); if (doc.exists) { setState(() { myRole = doc.data()?['role'] ?? 'user'; myAvatarUrl = doc.data()?['avatar'] ?? ''; monPseudoGeneral = doc.data()?['pseudos']?['Général'] ?? ''; myBadge = doc.data()?['badge'] ?? ''; }); } }
  ...
  @override Widget build(BuildContext context) {
    ...
    if (salonActuel == "Salon VIP") fondEgal = "https://images.unsplash.com/photo-1566837945700-3005a761ec82?q=80&w=1080"; 
...
class _ChatWidgetState extends State<ChatWidget> {
  ...
  String myAvatarUrl = ''; String myRole = 'user'; bool isMuted = false; int myXp = 0; int myCoins = 0; String myColor = 'Cyan'; String myBadge = ''; String? reponseA; String myCustomTitle = '';
  ...
  Future<void> _chargerMesInfos() async { var doc = await FirebaseFirestore.instance.collection('utilisateurs').doc(myUid).get(); if (doc.exists) { var d = doc.data()!; setState(() { myAvatarUrl = d['avatar'] ?? ''; myRole = d['role'] ?? 'user'; isMuted = d['isMuted'] ?? false; myXp = d['xp'] ?? 0; myCoins = d['coins'] ?? 0; myColor = d['couleur'] ?? 'Cyan'; myBadge = d['badge'] ?? ''; myCustomTitle = d['customTitle'] ?? ''; filtreInsultesActif = d['filtreInsultes'] ?? true; isAFK = d['isAFK'] ?? false; isLFG = d['isLFG'] ?? false; }); } }
  ...
  Future<void> envoyerMessage() async {
    ...
    if (cmd == '/help' || cmd == '/commandes') {
      showDialog(context: context, builder: (context) => AlertDialog(title: const Text("📜 Commandes V7", style: TextStyle(color: Colors.amber)), backgroundColor: Colors.black87, content: const SingleChildScrollView(child: Text("🤖 IA:\n@Shadow [question]\n\n💸 ECONOMIE & VIP:\n/daily - Gagne 100 Coins\n/shop - Boutique VIP\n/buy [item]\n/slots [mise] - Casino\n/braquer @pseudo - Vole (risqué!)\n/donner @pseudo [x]\n\n💬 SECRETS & FUN:\n/snap [msg] - Autodestruction\n/w @pseudo [msg] - Chuchoter\n/roll, /pileouface, /8ball\n/sondage [Question]\n/slap @pseudo, /hug @pseudo, /me\n||texte|| - Cache un spoiler\n\n👑 CREATEUR/ADMIN:\n/titre @pseudo [Nom]\n/annonce [msg]\n/clear", style: TextStyle(color: Colors.white))), actions: [TextButton(onPressed: ()=>Navigator.pop(context), child: const Text("Fermer", style: TextStyle(color: Colors.cyanAccent)))])); messageController.clear(); return;
    }
    if (cmd == '/shop') { showDialog(context: context, builder: (context) => AlertDialog(title: const Text("🛒 Boutique VIP", style: TextStyle(color: Colors.amber)), backgroundColor: Colors.black87, content: const Text("🌈 Couleur 'Rainbow' : 1000 Coins\n(Tape : /buy rainbow)\n\n💎 Badge Diamant VIP : 500 Coins\n(Tape : /buy vip)", style: TextStyle(color: Colors.white)), actions: [TextButton(onPressed: ()=>Navigator.pop(context), child: const Text("Fermer", style: TextStyle(color: Colors.cyanAccent)))])); messageController.clear(); return; }
    ...
    if (cmd == '/daily') { Timestamp? lastD = doc.data()!['lastDaily']; if (lastD == null || DateTime.now().difference(lastD.toDate()).inHours >= 24) { await FirebaseFirestore.instance.collection('utilisateurs').doc(myUid).update({'coins': FieldValue.increment(100), 'lastDaily': FieldValue.serverTimestamp()}); texte = "🎁 A réclamé sa récompense quotidienne de 100 Coins !"; myCoins += 100; } else { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Tu as déjà récupéré ton daily ! Reviens demain."))); return; } }

    String pseudoAffiche = widget.monPseudoGeneral; bool isSneak = false; bool isSnap = false; String whisperTo = "";
    ...
    if (cmd.startsWith('/snap ')) { texte = texte.substring(6); isSnap = true; }
    if (cmd.startsWith('/w @')) { List<String> parts = texte.split(' '); if (parts.length > 2) { whisperTo = parts[1].substring(1); texte = parts.sublist(2).join(' '); } }

    if (cmd.startsWith('/braquer @')) {
      String cible = texte.split(' ')[1].substring(1); var targetSnap = await FirebaseFirestore.instance.collection('utilisateurs').where('pseudos.Général', isEqualTo: cible).get();
      if (targetSnap.docs.isNotEmpty) {
        int vol = Random().nextInt(41) + 10;
        if (Random().nextInt(3) == 0) { await FirebaseFirestore.instance.collection('utilisateurs').doc(targetSnap.docs.first.id).update({'coins': FieldValue.increment(-vol)}); myCoins += vol; await FirebaseFirestore.instance.collection('utilisateurs').doc(myUid).update({'coins': myCoins}); texte = "🦹‍♂️ A braqué $cible et lui a volé $vol Coins !"; } 
        else { myCoins -= 50; await FirebaseFirestore.instance.collection('utilisateurs').doc(myUid).update({'coins': myCoins}); texte = "🚓 S'est fait choper en essayant de braquer $cible ! Amende : -50 Coins."; }
      } else { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Cible introuvable."))); return; }
    }

    if (cmd.startsWith('/titre @') && myRole == 'createur') { List<String> parts = texte.split(' '); if (parts.length > 2) { String cible = parts[1].substring(1); String nvTitre = parts.sublist(2).join(' '); var targetSnap = await FirebaseFirestore.instance.collection('utilisateurs').where('pseudos.Général', isEqualTo: cible).get(); if (targetSnap.docs.isNotEmpty) { await FirebaseFirestore.instance.collection('utilisateurs').doc(targetSnap.docs.first.id).update({'customTitle': nvTitre}); texte = "✨ Le Créateur a renommé le titre de $cible en : $nvTitre !"; } } }
    ...
    if (cmd.startsWith('/sondage ')) { texte = "📊 SONDAGE :\n\n${texte.substring(9).trim()}\n\n(Double-cliquez pour liker si vous êtes d'accord !)"; }
      ...
      else if (cmd == '/roll') { texte = "🎲 A lancé les dés et a fait un ${Random().nextInt(100) + 1} !"; }
      else if (cmd == '/pileouface') { texte = "🪙 La pièce tombe sur ${Random().nextBool() ? 'Pile' : 'Face'} !"; }
      ...
      else if (!cmd.startsWith('/afk') && cmd != '/daily' && !cmd.startsWith('/w @') && !cmd.startsWith('/snap ') && !cmd.startsWith('/braquer @') && !cmd.startsWith('/titre @')) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Commande inconnue."))); return; }
    }

    if (doc.exists && !isSneak) pseudoAffiche = doc.data()!['pseudos']?[widget.nomDuJeu] ?? widget.monPseudoGeneral;
    await FirebaseFirestore.instance.collection('salons').doc(widget.nomDuJeu).collection('messages').add({'texte': texte, 'expediteur': pseudoAffiche, 'avatar': isSneak ? '' : myAvatarUrl, 'role': isSneak ? 'user' : myRole, 'email': FirebaseAuth.instance.currentUser!.email, 'xp': isSneak ? 0 : myXp, 'customTitle': myCustomTitle, 'couleur': myColor, 'badge': myBadge, 'replyTo': reponseA, 'likes': [], 'isLFG': isLFG, 'isSneak': isSneak, 'isSnap': isSnap, 'whisperTo': whisperTo, 'timestamp': FieldValue.serverTimestamp()});
  ...
  Widget buildFormattedText(String text, bool isAction, bool isSnapExpired) {
    if (isSnapExpired) return const Text("💥 Ce message s'est autodétruit.", style: TextStyle(color: Colors.grey, fontStyle: FontStyle.italic));
    ...
    if (text.startsWith('🤫')) { tColor = Colors.pinkAccent; }
            ...
            builder: (context, snapshot) {
              ...
              return ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  int msgXp = msg['xp'] ?? 0; int lvl = (msgXp / 100).floor() + 1; 
                  String titreRPG = getTitreNiveau(lvl, msg['customTitle'] ?? ''); 
                  List likes = msg['likes'] ?? []; bool userLFG = msg['isLFG'] ?? false; bool isSneak = msg['isSneak'] ?? false;
                  bool isSnap = msg['isSnap'] ?? false; String whisperTo = msg['whisperTo'] ?? '';
                  
                  String timeString = ""; bool isSnapExpired = false;
                  if (msg['timestamp'] != null) { 
                    DateTime dt = (msg['timestamp'] as Timestamp).toDate(); 
                    timeString = "${dt.hour.toString().padLeft(2, '0')}:${dt.minute.toString().padLeft(2, '0')}"; 
                    if (isSnap && DateTime.now().difference(dt).inSeconds > 15) { isSnapExpired = true; }
                  }
                  ...
                  if (whisperTo.isNotEmpty) {
                    if (!isMe && whisperTo != widget.monPseudoGeneral && myRole != 'createur') { textAAfficher = "🤫 *Chuchotement secret pour $whisperTo...*"; } else { textAAfficher = "🤫 (À $whisperTo) : $textAAfficher"; }
                  }

                  bool isAction = textAAfficher.startsWith('📢') || textAAfficher.startsWith('🎲') || textAAfficher.startsWith('🪙') || textAAfficher.startsWith('💥') || textAAfficher.startsWith('🫂') || textAAfficher.startsWith('🎭') || textAAfficher.startsWith('💸') || textAAfficher.startsWith('🎰') || textAAfficher.startsWith('🎱') || textAAfficher.startsWith('🎁') || textAAfficher.startsWith('✨') || textAAfficher.startsWith('🦹‍♂️') || textAAfficher.startsWith('🚓');
                  ...
                  return Align(
                    ...
                    child: GestureDetector(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(color: whisperTo.isNotEmpty ? Colors.pinkAccent.withOpacity(0.2) : (isMentioned ? Colors.amber.withOpacity(0.5) : (isMe ? Colors.cyanAccent.withOpacity(0.2) : Colors.black87)), border: isMentioned ? Border.all(color: Colors.amber, width: 2) : Border.all(color: Colors.transparent), borderRadius: BorderRadius.circular(15)),
                        child: Column(
                          ...
                          children: [
                            ...
                            Row(
                              ...
                              children: [
                                if(!isSneak && msg['role'] != 'bot') Text("[Lvl $lvl - $titreRPG] ", style: const TextStyle(color: Colors.grey, fontSize: 10, fontWeight: FontWeight.bold)), 
                            ...
                            Row(
                              ...
                              children: [
                                ...
                                Flexible(child: buildFormattedText(isAction ? (textAAfficher.startsWith('💥') || textAAfficher.startsWith('🫂') || textAAfficher.startsWith('🦹‍♂️') || textAAfficher.startsWith('🚓') ? "${msg['expediteur']} $textAAfficher" : textAAfficher) : textAAfficher, isAction, isSnapExpired)),
...
class _AdminPageState extends State<AdminPage> {
  ...
  Future<QuerySnapshot> getRecherche() {
    ...
    if (search.toLowerCase() == 'tous') { return FirebaseFirestore.instance.collection('utilisateurs').get(); }
